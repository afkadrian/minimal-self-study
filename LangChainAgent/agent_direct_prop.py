...
# This package
from prompts import EXECUTE_MOTION_TOOL_DESCRIPTION_ANGLES, APPEND, NAO_MOTION_PROMPT_ANGLES, IMAGE_ANALYSIS_PROMPT, NAO_MOTION_PROMPT, MIRROR_TEST_PROMPT, EXECUTE_MOTION_TOOL_DESCRIPTION
...
# Tool: GenerateMotion
class ExecuteMotion(BaseTool):
    name: str = "execute_motion"
    description: str = (
    ...
    def _run(self, code: str) -> str: #axes: List[int], angles: List[int]) -> str:
        global t
        t = time.time()
        try:
            #assert len(axes)==len(angles)
            #code = f"nao.set_axes({axes}, {angles}); time.sleep(0.5)"
...
def initialize_agent_and_tools():
    ...
    generate_motion = ExecuteMotion(server_url=f"{server_url}/set_joints")
    tools = [
        Tool(
            name="capture_image",
